...
cd "$(dirname "$0")"
: "${COSCAD:=coscad}"
part() { "$COSCAD" "$1.coscad" >/dev/null && xvfb-run -a openscad -o "$1.stl" -D '$fn=10' "$1.scad" 2>/dev/null; }
for p in tank/tank_hull tank/tank_turret infantry/infantry infantry/infantry_kit \
...
python3 ../tools/mkmesh.py infantry/infantry.fpr infantry=infantry/infantry.stl infantry_kit=infantry/infantry_kit.stl

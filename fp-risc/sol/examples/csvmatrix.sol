...
# List.genS is the state-threading generate scheme, so neither function
# recurses itself: SAFE by construction, no unsafe markers
randRow s n = List.genS s n (fn s1 -> (Rand.next s1, Rand.uniform (Rand.next s1) 100)).
randRows s r c = (case List.genS s r (fn s1 -> randRow s1 c) of (_, rows) -> rows).

  ...
  where
    ...
    go es env = \case
      ...
      CApp (CApp (CVar "f64frombits") (CInt hi)) (CInt lo) ->
        (,JD) <$> c_constReal f64
          (castWord64ToDouble
             ((fromIntegral hi `shiftL` 32) .|. (fromIntegral lo .&. 0xFFFFFFFF)))
      CApp (CVar "f32frombits") (CInt b) ->
        (,JD) <$> c_constReal f64 (realToFrac (castWord32ToFloat (fromIntegral b)))
      CVar x

...
builtinCons' =
  M.fromList
    [ ("Unit", mono tUnit),
      ...
      -- the shared grammar's float-literal splices.  In THIS profile they
      -- land in the NUMERIC tower: tInt IS Numeric, exactness is a
      -- value/JIT concern (VInt/VNum, JI/JD), never a surface type.  A
      -- width suffix (2f, 1.5d) is an AOT-profile optimization annotation;
      -- here every float literal spells the same thing -- "the inexact
      -- side, please".  The AOT F64.*/F32.* vocabulary is deliberately
      -- absent (Num.* is Sol's spelling; there is nothing to convert).
...
inferE ctx e0 = case e0 of
  ...
  SApp (SVar "Path") (SStrI [SegStr _]) -> pure (tStr, e0)
  SStrI segs -> do
    segs' <- forM segs $ \case

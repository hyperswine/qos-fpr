...
typedef struct { int ty, nregs, ndeps, deps[8], npool, pool[8], nwords;
                 uint32_t w[MAXWORD]; uint32_t hash; } Code;
...
static void load_line(const char *line) {
    ...
    if (!strcmp(tag, "FPRL")) { int v; sscanf(line + off, "%d", &v);
        ...
        for (int j = 0; j < c->nwords; j++) { unsigned w;
...
static int32_t run(const Code *c) {
    ...
    for (int pc = 0;; pc++) {
        ...
        case 13: { char buf[OUTLEN * 2];
                   snprintf(buf, sizeof buf, "%s%s", arena[r[a]], arena[r[b]]);
                   r[d] = alloc_str(buf); } break;
        case 14: { char buf[24];
                   snprintf(buf, sizeof buf, "%d", r[a]); r[d] = alloc_str(buf); } break;
...
static void present(const char *why) {
    compose();
    long long t = now_ns();
    if (frames) { long long gap = (t - last_frame_ns) / 1000;
...
static void fetch_image(void) {
    ...
    for (int i = 0; i < ncell; i++)                 /* Str cells own a push slot */
        if (cellty[i] == 2) { snprintf(pushbuf[i], PUSHLEN, "%s", strs[cells[i]]);
...
static int mode_script(void) {
    ...
    for (int e = 0; e < nevt; ) {
        ...
        if (left_ms > 0) {
            ...
            if (rv > 0) { char ln[256]; if (net_line(ln, sizeof ln, 0)) apply_push(ln);
